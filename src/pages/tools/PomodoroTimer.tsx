...
const PomodoroTimer = () => {
  ...
  useEffect(() => {
    ...
    const description = "Focus with configurable Pomodoro work and break sessions. Track completed sessions with simple controls and gentle sound cues.";
    setMetaDescription(description);
    const keywords = "Pomodoro timer,focus timer,productivity,work sessions,break timer";
    ...
    const breadcrumbJson = {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": "Pomodoro Timer", "item": canonicalHref }
      ]
    };
    const webPageJson = {
      "@context": "https://schema.org",
    ...
    const faqJson = {
      ...
      "mainEntity": [
        ...
        },
        {
          ...
          "acceptedAnswer": {
            "@type": { "@type": "Answer", "text": "Yes. Use the settings to adjust work and break lengths to suit your routine." }
          }
        },
        {
          "@type": "Question",
  ...
  const deleteTask = (id: string) => {
    ...
    if (currentTaskId === id) setCurrentTaskId(tasks.length > 1 ? tasks[0].id : null);
  };
  ...
  // --- Timer Logic ---
  const getCurrentDuration = useCallback((m: typeof mode) => {
    switch (m) {
  ...
  const handleSessionEnd = useCallback(() => {
    ...
    if (mode === "work") {
      ...
      if (currentTaskId) {
        setTasks(prev => prev.map(t => t.id === currentTaskId ? { ...t, actPomodoros: t.actPomodoros + 1 } : t));
      }
  ...
  const finishTime = new Date(Date.now() + (totalEstPomodoros - totalActPomodoros) * settings.workDuration * 60000);
        ...
        <div className="container mx-auto px-4 max-w-[1000px]">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-bold text-foreground">Pomodoro Timer</h1>
            </div>
              ...
              <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="space-y-6 py-4">
                  <div className="space-y-4">
                    <Label className="text-base font-bold">Durations (minutes)</Label>
                  ...
                  <div className="space-y-4 pt-4 border-t">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(v) => setSettings({ ...settings, autoStartBreaks: v })}
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(v) => setSettings({ ...settings, autoStartPomodoros: v })}
                      />
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 gap-8">
            {/* Timer Card */}
            <div className={`bg-white rounded-3xl border-2 shadow-xl overflow-hidden transition-all duration-500 ${
              mode === "work" ? "border-primary/20" : mode === "short_break" ? "border-blue-400/20" : "border-indigo-400/20"
            }`}>
              <div className="p-8 md:p-12 flex flex-col items-center">
                ...
                <div className="relative w-64 h-64 md:w-80 md:h-80 flex items-center justify-center mb-12">
                  <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 200 200">
                    <circle cx="100" cy="100" r="92" stroke="currentColor" strokeWidth="8" fill="none" className="text-slate-100" />
                    <circle
                      cx="100"
                      cy="100"
                      r="92"
                      ...
                      className={`transition-all duration-1000 ${
                        mode === "work" ? "text-primary" : mode === "short_break" ? "text-blue-500" : "text-indigo-500"
                      }`}
                    />
                  </svg>
                ...
                <div className="flex items-center gap-6">
                  ...
                  >
                    {isRunning ? (
                      <span className="flex items-center gap-3"><Pause className="h-6 w-6 fill-current" /> PAUSE</span>
                    ) : (
                      <span className="flex items-center gap-3"><Play className="h-6 w-6 fill-current" /> START</span>
                    )}
                  </Button>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between pb-2 border-b-2 border-slate-200">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    <DropdownMenuItem onClick={clearCompletedTasks} className="text-red-600">
                      <Trash2 className="h-4 w-4 mr-2" />
                      Clear Completed
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setTasks([])} className="text-red-600 font-bold">
                      <Trash2 className="h-4 w-4 mr-2" />
                      Clear All Tasks
                    </DropdownMenuItem>
                  </DropdownMenuContent>
              ...
              <div className="space-y-3">
                {tasks.map((task) => (
                  <div
                    ...
                    className={`group relative bg-white p-5 rounded-2xl border-2 transition-all cursor-pointer hover:border-primary/30 ${
                      currentTaskId === task.id ? "border-primary ring-4 ring-primary/5" : "border-transparent"
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <button
                          ...
                          className={`h-7 w-7 rounded-full border-2 flex items-center justify-center transition-all ${
                            task.completed ? "bg-primary border-primary text-white" : "border-slate-200 text-transparent hover:border-primary"
                          }`}
                        >
                          <CheckCircle2 className="h-4 w-4" />
                        </button>
                        <span className={`font-bold text-lg ${task.completed ? "line-through text-slate-400" : "text-slate-700"}`}>
                          {task.title}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-slate-400 font-bold tabular-nums">
                          <span className="text-slate-900 text-xl">{task.actPomodoros}</span>
                          ...
                          <DialogContent>
                            ...
                            <form
                              onSubmit={(e) => {
                                ...
                                const est = parseInt(formData.get("est") as string) || 1;
                                if (title) {
                                  updateTask(task.id, { title, estPomodoros: est });
                            ...
                            >
                              <div className="space-y-2">
                                ...
                                <Input id="edit-task-title" name="title" placeholder="Task name..." defaultValue={task.title} required autoFocus />
                              </div>
                              <div className="space-y-2">
                                <Label htmlFor="edit-task-est">Estimated Pomodoros</Label>
                                <div className="flex items-center gap-4">
                                  <Input id="edit-task-est" name="est" type="number" defaultValue={task.estPomodoros} min={1} className="w-24" />
                                  <span className="text-sm text-slate-400 font-medium">approx. {(task.estPomodoros * settings.workDuration)} mins</span>
                                </div>
                              </div>
                              <DialogFooter>
                ...
                <Dialog open={addTaskDialogOpen} onOpenChange={setAddTaskDialogOpen}>
                  ...
                  <DialogContent>
                    ...
                    >
                      <div className="space-y-2">
                        <Label htmlFor="task-title">What are you working on?</Label>
                        <Input id="task-title" name="title" placeholder="Task name..." required autoFocus />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="task-est">Estimated Pomodoros</Label>
                        <div className="flex items-center gap-4">
                          <Input id="task-est" name="est" type="number" defaultValue={1} min={1} className="w-24" />
                          <span className="text-sm text-slate-400 font-medium">approx. {(1 * settings.workDuration)} mins</span>
                        </div>
                      </div>
                      <DialogFooter>
              ...
              <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-xl flex flex-wrap justify-between items-center gap-6">
                <div className="flex gap-8">
                  <div>
                    <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Pomos</p>
                    <p className="text-2xl font-black tabular-nums">{totalActPomodoros}<span className="text-slate-500 mx-1">/</span>{totalEstPomodoros}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Finish At</p>
                ...
                <div className="px-4 py-2 bg-white/10 rounded-xl backdrop-blur-md">
                  <p className="text-xs font-bold text-primary-foreground/70 uppercase tracking-widest">Total Focus</p>
                  <p className="text-xl font-black">{totalActPomodoros * settings.workDuration}m</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-20">
            <section className="bg-white border rounded-3xl p-8 shadow-sm">
              ...
              <div className="space-y-4 text-slate-600 leading-relaxed font-medium">
                <p>
                  Developed by Francesco Cirillo in the late 1980s, this method uses a timer to break work into intervals, traditionally 25 minutes in length, separated by short breaks.
                </p>
            ...
            <section className="bg-white border rounded-3xl p-8 shadow-sm">
              <h2 className="text-2xl font-black text-slate-800 mb-6">Pro Tips</h2>
              <div className="space-y-6">
                <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  ...
                  <p className="text-blue-700/80 text-sm font-medium">
                    When the timer goes off, stop working immediately. Breaks are essential for long-term focus and preventing burnout.
                  </p>
                </div>
                <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                  ...
                  <p className="text-indigo-700/80 text-sm font-medium">
                    If you have several tiny tasks (like replying to emails), group them into a single Pomodoro session.
                  </p>
                </div>
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-slate-700/80 text-sm font-medium">
                    Over time, you'll get better at estimating how many Pomodoros a task takes. This improves your planning skills!
                  </p>
                </div>
              </div>
            </section>
